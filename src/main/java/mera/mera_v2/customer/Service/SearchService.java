...
public class SearchService {
    ...
    @PostConstruct
    public void init() {
        log.info("SearchService initialized - doc mapping tu file JSON (khong dung DB)");
    }
    ...
    public Map<String, Object> searchCustomer360(String phone) throws Exception {
        ...
        List<UserConfigDto> userConfigs = loadConfigsFromMappingFile();
        if (userConfigs == null || userConfigs.isEmpty()) {
            log.warn("Khong co config nao trong file mapping ({})", cskhMappingService.getLoadedFrom());
        }
    ...
    /**
     * Nguon config cua trang search-info: file mapping CSKH (thay cho bang search_config).
     * Lay TAT CA entry, ke ca base he thong (khong co SDT/CSKH) — search-info can quet het cac base.
     */
    private List<UserConfigDto> loadConfigsFromMappingFile() {
        List<CskhMapping> mappings = cskhMappingService.getAllIncludingSystemBases();
    ...
    private UserConfigDto toUserConfigDto(CskhMapping cfg) {
        UserConfigDto dto = new UserConfigDto(null, null);
        dto.setBaseId(cfg.getBaseId());
        dto.setLarkName(cfg.getBaseName());
        // posName rong => base he thong: quet bang dong + dung tenant token (xem fetchLarkExchanges)
        // specialWarningName => hien canh bao "Khach hang nam trong bang X"
        if (cfg.getSpecialWarningName() != null && !cfg.getSpecialWarningName().isBlank()) {
            dto.setPosName("__SPECIAL__:" + cfg.getSpecialWarningName().trim());
        } else {
            dto.setPosName(cfg.getCskhName());
        }
