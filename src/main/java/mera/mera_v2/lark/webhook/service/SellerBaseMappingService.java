...
public class SellerBaseMappingService {
    ...
    public Optional<String> findBaseIdBySellerName(String sellerName) {
        ...
        String baseId = SELLER_BASE_MAP.get(sellerName.trim());
        if (baseId != null) {
            log.debug("Found Base ID for seller '{}': {}", sellerName, baseId);
            return Optional.of(baseId);
        }
        
        for (Map.Entry<String, String> entry : SELLER_BASE_MAP.entrySet()) {
            if (entry.getKey().equalsIgnoreCase(sellerName.trim())) {
                log.debug("Found Base ID (case-insensitive) for seller '{}': {}", sellerName, entry.getValue());
                return Optional.of(entry.getValue());
            }
        }
        
        log.warn("⚠️ No Base ID found for seller name: '{}'", sellerName);
        return Optional.empty();
    }
